...
const Input = ({
  ...
  return (
    <>
      <label
        className={`text-body font-normal ${isFirst ? '' : 'mt-2.5'} mb-1.5 text-gray-900`}
        htmlFor={name}
      >
        {label}
      </label>
      ...
      />
      {fieldError && (
        <p className="text-red-600 text-label mt-0 mb-2">
          {fieldError.message}
        </p>
      )}
    </>
  ...
  return (
    <div className="flex-1 min-w-form-min py-12 px-10 flex flex-col justify-center">
      <form className="flex flex-col mt-6" onSubmit={handleSubmit(onSubmit)}>
        ...
        {errorLog && (
          <p className="text-red-600 text-center mb-3 text-label">
            {' '}
            {errorLog}{' '}
          </p>
        )}
